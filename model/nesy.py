...
class NeSyLoss():
    ...
    def __init__(self, sat_p: int = 2, uq_p: int = 2, pos_idx: int = 2, strategy: int = 0) -> None:
        """create the neuro-symbolic loss

        Args:
            sat_p (int, optional): aggregator p-mean norm value for satisfiability computation. Defaults to 2.
            uq_p (int, optional): aggregator p-mean norm value for universal quantification computation. Defaults to 2.
        ...
        self.p_is_similar = ltn.Predicate(
            func=lambda anchor, candidate: 0.5 + F.cosine_similarity(anchor, candidate, dim=-1, eps=1e-4)*0.5)
        self.p_is_score = ltn.Predicate(
            func=lambda pred, tgt: torch.sum(pred * tgt, dim=-1))

        self.uq_p = uq_p
        self.forall = ltn.Quantifier(
            ltn.fuzzy_ops.AggregPMeanError(p=uq_p), quantifier='f')
        self.implies = ltn.Connective(ltn.fuzzy_ops.ImpliesReichenbach())
        self.sat_axiom = ltn.fuzzy_ops.SatAgg(
            ltn.fuzzy_ops.AggregPMeanError(p=sat_p))
    ...
    def increase_pmean(self, factor: int = 2) -> None:
        ...
        self.forall.agg_op.p = self.uq_p
        ...
        if self.strategy == 0:
            ...
            out = self.forall(ltn.diag(anchor_emb_var, cand_emb_var, pred_scores_var, tgt_scores_var),
                              self.implies(self.p_is_similar(anchor_emb_var, cand_emb_var),
                                           self.p_is_score(
                                  pred_scores_var, tgt_scores_var)
            )
            )
            ...
            pos_idx_t = torch.tensor(
                [True if t.argmax() >= self.pos_idx else False for t in tgt_scores])
            neg_idx_t = torch.tensor(
                [True if t.argmax() < self.pos_idx else False for t in tgt_scores])

            if torch.sum(pos_idx_t) > 0:
                has_pos = True

                anchor_emb_var_p = ltn.Variable(
                    'anchor_emb_p', anchor_emb[pos_idx_t])
                cand_emb_var_p = ltn.Variable(
                    'anchor_emb_p', cand_emb[pos_idx_t])

                pred_scores_var_p = ltn.Variable(
                    'pred_score_p', pred_scores[pos_idx_t])
                tgt_scores_var_p = ltn.Variable(
                    'tgt_score_p', tgt_scores[pos_idx_t])
            ...
            if torch.sum(neg_idx_t) > 0:
                has_neg = True
                anchor_emb_var_n = ltn.Variable(
                    'anchor_emb_n', anchor_emb[neg_idx_t])

                # print(f'size\tcand:{anchor_emb.size()} neg_idx_t{neg_idx_t.size()}')
                # print(f'size\tcand:{cand_emb.size()} neg_idx_t{neg_idx_t.size()}')

                cand_emb_var_n = ltn.Variable(
                    'cand_emb_n', cand_emb[neg_idx_t])

                pred_scores_var_n = ltn.Variable(
                    'pred_score_n', pred_scores[neg_idx_t])
                tgt_scores_var_n = ltn.Variable(
                    'tgt_score_n', tgt_scores[neg_idx_t])
            ...
            if self.strategy == 1:
                if has_pos:
                    ...
                    check_val = self.p_is_similar(
                        pred_scores_var_p, tgt_scores_var_p).value
                    assert (check_val <= 1.0).all(
                    ), f'{check_val} is not in range'
                    ...
                    out_pos = self.forall(ltn.diag(anchor_emb_var_p, cand_emb_var_p, pred_scores_var_p, tgt_scores_var_p),
                                          self.implies(self.p_is_similar(anchor_emb_var_p, cand_emb_var_p),
                                                       self.p_is_score(
                                              pred_scores_var_p, tgt_scores_var_p)
                    )
                    )
                ...
                if has_neg:
                    # sanity check to have useful trackback in case of dumb predicate error
                    check_val = self.p_is_similar(
                        pred_scores_var_n, tgt_scores_var_n).value
                    assert (check_val <= 1.0).all(
                    ), f'{check_val} is not in range'
                    ...
                    out_neg = self.forall(ltn.diag(anchor_emb_var_n, cand_emb_var_n, pred_scores_var_n, tgt_scores_var_n),
                                          self.implies(self.p_is_similar(anchor_emb_var_n, cand_emb_var_n),
                                                       self.p_is_score(
                                              pred_scores_var_n, tgt_scores_var_n)
                    )
                    )
